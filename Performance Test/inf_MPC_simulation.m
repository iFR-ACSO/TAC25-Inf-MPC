...
dt0         = 0.1;
simTime     = 10000; % maxTime allowed for simulation
simStepSize = dt0;
...
numRuns  = 100;       % Define number of Monte-Carlo runs
startSim = tic;
...
% get 1000 initial attitude; rates set to zero because rest-to-rest
a4 = -1;  b4 = 1;
x0_low(4:6,:) = (b4-a4)*rand(3,1000)+a4;
...
if numRuns > 1
    ...
    % reduce to feasible initial conditions to no. of runs
    x0_low = x0_low(:,idx);
    ...
    for k = 1:numRuns
        b_I0 = mrp2trafo_BI(x0_low(4:6,k))'*b_B;
...
for j = 1:numRuns
    ...
    for k = 2:simTime/simStepSize
        ...
        if norm(x_sol_vec_infMPC(1:3,k),inf)*180/pi < 1e-3 && ... % below 0.001 deg/s
           norm([phi,theta,psi],inf) < 0.3 &&...                  % below 0.3 deg
...
save('infiniteMPC_keepOut_Results.mat',...
    'Q_infinite','R_infinite','tEnd_all_inf',...
    'x_sol_vec_infMPC','u_sol_vec_infMPC',...
    'u_low','u_up','Omega_bounds',...
    "simTime","simStepSize","iter_conv_inf","maxSolveTime_infMPC")
