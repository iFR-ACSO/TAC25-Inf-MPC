...
%                      full horizon NMPC. The problem is setup
%                      using CasADi. We make use of CasADi's nlsol 
%                      interface. Different simulation modes are possible 
%                      (single and comparison). We make use of a
...
% select a NLP solver method; for SQP the QP solver qrqp is used but can be
% also changed
solvermethod = 'fatrop'; % 'IPOPT'; 'fatrop'; 'alpaqa'; 'SQP'
...
if strcmp(SimulationType,'Single')
    ...
    numRuns  = 1;       % Define number of Monte-Carlo runs
     ...
     % .mat file name
     matFileName = 'fullhorizon_IPOPT_comparison.mat';
    

     % define integration step size for different initial conditions
        h_vec = 4;
        T_vec = 400;         % prediction horizon
    ...
    % define integration step size for different initial conditions
    h_vec = [4; 3; 1];
    T_vec = [400; 300; 100];         % prediction horizon (heuristically found!)

    
     % .mat file name
     matFileName = 'fullhorizon_IPOPT_comparison_3_runs.mat';
end
...
elseif strcmp(solvermethod,'fatrop')    

     options = struct('fatrop',struct('print_level',0), ...
                      'print_time',false,...
                      'record_time',true);

    solver = casadi.nlpsol('solver', 'fatrop', prob,options);

elseif strcmp(solvermethod,'ipopt')  
...
save('full_horizon_MPC_simulation_completeWS_3_runs.mat')
